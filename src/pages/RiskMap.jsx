...
function getRiskIcon(risk = "Advisory") {
  return L.divIcon({
    ...
    html: `
      <span class="leaflet-risk-marker risk-${String(
        risk
      ).toLowerCase()}"></span>
    `,
...
function getAlertMessage(level, isHindi) {
  if (isHindi) {
    ...
    if (level === "Elevated") {
      return "बढ़ा हुआ ML खतरा संकेत मिला है। संवेदनशील क्षेत्रों पर निगरानी रखें।";
    }
    return "वर्तमान ML संकेत निम्न स्तर पर है। नियमित निगरानी जारी रखें।";
  }

  if (level === "High") {
    return "High ML hazard signal detected. Increase preparedness immediately and follow local authority instructions.";
  }
  if (level === "Moderate") {
    return "Moderate ML hazard signal detected. Increase monitoring of rainfall, soil and local conditions.";
  }
  if (level === "Elevated") {
    return "Elevated ML hazard signal detected. Maintain close monitoring of vulnerable areas.";
  }
  return "Current ML signal is low. Continue routine monitoring.";
}
...
function RiskMap() {
  ...
  // AUTOMATIC ALERT ENGINE
  //
  // The engine evaluates the highest ML hazard signal returned
  // by the backend. No manual "send alert" action is required.
  // ==========================================================
  ...
  const automaticAlertCandidate = useMemo(() => {
    ...
    const candidates = riskZones
      .map((zone) => {
        ...
        const available =
          zone?.ml_prediction?.available === true;

        if (!available || !Number.isFinite(score)) {
          return null;
        }
      ...
      .sort((a, b) => {
        ...
        return (
          getMLHazardPriority(b.level) -
          getMLHazardPriority(a.level)
        );
      })[0] || null;
  ...
  useEffect(() => {
    ...
    // Only elevated-or-higher ML signals create an alert.
    if (getMLHazardPriority(level) < 1) {
      setAutomaticAlert(null);
      lastAlertKeyRef.current = null;
      return;
    }
    ...
    // Do not repeatedly play the siren while polling the same state.
    if (
      alertSoundEnabled &&
      ...
      try {
        ...
        if (AudioContextClass) {
          ...
          [0, 0.32, 0.64].forEach((offset) => {
            ...
            oscillator.stop(now + offset + 9.0);
          });
        ...
        console.warn(
          "Automatic alert sound unavailable:",
          error
        );
      }
    }
              ...
              >
                ...
                >
                  <button
                    ...
                    title={
                      isHindi
                        ? "स्वचालित सायरन चालू/बंद करें"
                        : "Toggle automatic siren"
                    }
                  >
                    {alertSoundEnabled
                      ? isHindi
